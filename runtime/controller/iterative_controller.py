...
class IterativeController(AController):

    def search(self, number_of_episodes, model_reference: AModelReference):
        initial_executable_model = self._model_factory.to_executable_model(model_reference)
        reference_evaluation = self._model_evaluator.evaluate(initial_executable_model, list()) | self._lat_eval(
            initial_executable_model)
        ...
        model_features = self._feature_extractor.extract_metrics_and_sens(initial_model_for_features, model_reference)
        current_policy = self._reference_policy
        ...
        for episode in tqdm(range(number_of_episodes), desc="[Search Policies - Episodes]"):
            ...
            for step in tqdm(range(self._steps_per_episode), desc="[Episode Steps]"):
                ...
                predicted_policy, raw_actions = self._agent_wrapper.predict_policy(global_step, model_features,
                                                                                   current_policy)

                if predicted_policy is not None:
                    current_policy = predicted_policy
                    executable_model, step_evaluation = self._compress_and_evaluate(current_policy, model_reference)
                    model_features = self._feature_extractor.extract_metrics_and_sens(executable_model,
                                                                                      model_reference)
                    self._agent_wrapper.pass_step_results(global_step, step_evaluation, model_features)
                    self._logging_service.step_results(step_evaluation, raw_actions)

            episode_model, episode_evaluation = self._compress_and_evaluate(current_policy, model_reference,
                                                                            episode=True)
